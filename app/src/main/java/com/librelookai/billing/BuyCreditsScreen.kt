    ...
    Box(modifier = modifier.fillMaxSize()) {
        ...
        state.successMessage?.let { msg ->
            Snackbar(
                ...
                action = { TextButton(onClick = creditsViewModel::clearSuccess) { Text(stringResource(R.string.action_ok)) } },
            ) { Text(msg) }
        }
        ...
        state.error?.let { msg ->
            Snackbar(
                modifier = Modifier.align(Alignment.BottomCenter).padding(16.dp),
                action = { TextButton(onClick = creditsViewModel::clearError) { Text(stringResource(R.string.action_dismiss)) } },
            ) { Text(msg) }
        }
    }
}
