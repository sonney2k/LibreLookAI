...
internal fun WardrobeViewModel.openCapture() = _state.update { it.copy(view = WardrobeView.CAPTURE) }
internal fun WardrobeViewModel.closeCapture() = _state.update { it.copy(view = WardrobeView.GRID) }

    /** Open the camera in "find item by photo" mode. */
internal fun WardrobeViewModel.openFindByPhoto() = _state.update { it.copy(view = WardrobeView.FIND_BY_PHOTO_CAPTURE) }

    /** User cancelled the find-by-photo capture without taking a photo. */
internal fun WardrobeViewModel.closeFindByPhoto() = _state.update { it.copy(view = WardrobeView.GRID) }
     ...
     */
internal fun WardrobeViewModel.onFindByPhotoCaptured(rawFile: File) {
...
internal fun WardrobeViewModel.dismissFindByPhoto() {
     ...
     */
internal suspend fun WardrobeViewModel.findSimilarInCandidates(
        rawFile: File,
...
internal fun WardrobeViewModel.searchByText(query: String) {
        val q = query.trim()
        if (q.isEmpty()) return
        viewModelScope.launch {
            _state.update { it.copy(
...
internal fun WardrobeViewModel.fuzzyFilterByText(query: String, items: List<DriveImage>): List<DriveImage> {
        val q = query.trim()
        if (q.isEmpty()) return items
        return fuzzyMatchByTags(q, items).map { it.image }
    }

internal fun WardrobeViewModel.fuzzyMatchByTags(query: String, items: List<DriveImage>): List<FindByPhotoMatch> {
...
internal fun WardrobeViewModel.bestTokenScore(token: String, tagStrings: List<String>): Float {
        var best = 0f
...
internal fun WardrobeViewModel.normalizeForSearch(s: String): String = s.lowercase()
...
internal fun WardrobeViewModel.localizedResourceContext(): Context {
...
internal fun WardrobeViewModel.tagsToStrings(t: ClothingTags): List<String> = buildList {
...
internal fun WardrobeViewModel.levenshtein(a: String, b: String): Int {
