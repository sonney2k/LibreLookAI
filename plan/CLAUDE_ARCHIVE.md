...
- **Two-phase load**: Phase 1 shows the local cache instantly (zero network). Phase 2 fetches Drive listing + sidecars in parallel and rebuilds `wardrobe_cache_{folderId}.json`. `_wardrobe_metadata.json` is read as a one-time fallback when no sidecars exist; items are migrated fire-and-forget on first load.
  - **Closet-switch perf**: Phase 1's cache read + Gson parse (single closet and the All-locations merge), plus the cross-closet `refreshAllLocationImagesState` snapshot, run on `Dispatchers.IO`, not the main thread — `viewModelScope` defaults to `Main`, so parsing a large closet's JSON on switch (or on every tag edit, which calls `saveLocalCache` → snapshot refresh) used to jank the UI. The snapshot refresh is coalesced (`snapshotJob.cancel()`), since each launch re-reads the latest cache from disk. Warming: `prefetchUncachedClosets` (triggered by `setAllConfiguredLocations` during restore) downloads + caches every still-uncached closet so later switches hit Phase 1 instantly; `retryPrefetchIfNeeded` re-runs it when connectivity returns.
- **Optimistic writes (invariant)**: every mutation — move, delete, tag edit, and the bg-removal queue's final persist — updates in-memory state **and** the local cache **synchronously, before** the Drive round-trip; Drive syncs in the background and rolls back on failure. This keeps add/move/delete/edit instant and flicker-free.
  - **Move** (`WardrobeViewModel.moveItemsToLocation`, `ShoppingClosetViewModel.moveToCloset`): drop from the source view+cache and register in the target via `notifyItemsMovedTo` up front, then `moveFile` in the background; failures call `rollbackMove` / `undoItemsMovedTo` (target-cache + `recentlyMovedItems` undo, source restore). This is what makes "move, then switch closet" appear instantly instead of waiting ~1 min for Drive's eventually-consistent listing. `recentlyMovedItems` (TTL 10 min) is dual-purpose: Phase 2 **re-injects** moved items into the *target* folder and **suppresses** them from the *source* folder — without the suppression, Drive still lists the item under its old parent for minutes (testers saw ~5 min), resurrecting the stale copy and showing it in both closets.
  - **Cache-folder rule**: a cache write must always be keyed off the item's **owning** `folderId`, never the transient active-filter `folderId` (which is `null` in All-locations mode and changes when the user switches closets). Use the file-level helpers `persistItemToCache` / `addToCacheFile` / `removeFromCacheFile` (edit the target folder's cache file directly) rather than rebuilding a cache from the whole `state.images`. The bg queue persists each finished item via `persistItemToCache(job.folderId, …)` for exactly this reason.
  - **Outfits** mirror the same invariant: every `OutfitsViewModel` mutation calls `refreshOutfitsLocalCache()` after the state update so `styles_cache_{folderId}.json` never resurrects a deleted/edited outfit on the next cold start. **Trips** already write `trips_cache.json` after every `upsert/mutate/delete`.
...
- `Layer` enum (Outerwear / Top / OnePiece / Bottom / Footwear / Accessory) lives in the same file; `layerFor(DriveImage)` classifies items by tag category regex. `OnePiece` covers `dress`, `suit`, `jumpsuit`, `gown`, `romper` — anything that occupies both top and bottom slots.
- **One-piece ↔ Top/Bottom exclusion** (`OutfitSlot.kt` helpers): an Einteiler covers BOTH `Top` and `Bottom`, so they're mutually exclusive. `onePieceConflict(a,b)` + `setSlotItem` clear the opposing side when the user picks one (just-touched slot wins). `normalizeOnePieceExclusion(slots)` is the bulk/AI safety net (locked side wins, else filled one-piece wins). `collapseOnePieceSlots(slots)` hides the empty slots a filled one-piece makes redundant — used for the look-board `visibleSlots` AND for `filledSlots`/`totalSlots`/`saveEnabled`/`aiCanGenerate` so a dress (+ shoes) reads as complete with no dangling empty Top/Bottom rows. The composer prompt also states the rule so Gemini never pairs a dress with a separate top/bottom.
- **One-piece "core band"**: in `ComposerStackedView` the look board sizes tiles off a square `unit`; the OnePiece tile is **2 vertical units** tall (`unit * 2 + rowGap`). When a one-piece coexists with the Top/Bottom slots it covers (`coreFlank`), `ComposerCoreBand` lays them side by side: the suit is one full-height rectangle that can be flanked by a `ComposerFlankColumn` on **each side** (Top/Bottom tiles are split left/right, left-biased — a single top/bottom stays left, the next appears right). In each flank column the Top tile is pinned to the band's **top** edge and the Bottom tile to its **bottom** edge (a `weight(1f)` Spacer pushes them apart) — *not* stacked above/below the suit. When the one-piece is alone (Top/Bottom collapsed away), it just renders as a tall 2-unit tile. `ComposerStackedTile` takes `width`/`height` (not a single `size`) and scales its icons/badges off `width`.
...
- **Closet scoping (list + viewer)**: the travel list and the trip viewer resolve trip items from `WardrobeUiState.allLocationImages` (the cross-closet snapshot), **not** the active-closet-scoped `images` — MainActivity trims `images` to the active closet via `setLocation`, so resolving against it blanks any item that lives in another closet. The travel-list closet filter keys on each **item's `folderId`** (a trip shows under closet X when all its items live in X), **not** `Outfit.folderId` (the outfit-JSON storage folder, which can differ from where the items are and previously made closet-scoped trips render empty or vanish). Trips/orphan outfits with no resolvable items are dropped; while the snapshot is still loading (`allLocationImages` empty) the list isn't blanked.
...
**Pricing**: **EUR-native** (Google bills the EU account in EUR — no USD ground truth, no FX). Rates come from `ModelPricingClient` (Firestore `config/modelPricing` listener → SharedPrefs cache → `DefaultModelPricing` fallback). The schema splits rates by token type (`textInPerM` / `imageInPerM` / `cachedTextInPerM` / `textOutPerM` / `imageOutPerM`) to match the bill — see `plan/FIN.md` § "Gemini pricing model & unit economics". `GeminiPricing.eurFor(model, in, out)` bills input at the text-in rate (text-in == image-in today) and output at the image-out rate for image-generation models (`ModelPricingClient.isImageOutputModel` — name contains "image"), else text-out. `UsageEvent.eurAtRecord` snapshots € at record time so old events keep their historical price; pre-EUR events carried a legacy `"usd"` key (no longer read) and fall back to current rates. `cachedTextInPerM` is recorded for completeness but unused — context caching is not implemented (media-gen models can't cache; see `plan/FIN.md` § "Gemini pricing model & unit economics", "Context caching" subsection).
...
**Scope is a build flag.** `BuildConfig.DRIVE_FULL_SCOPE` (← `local.properties` `drive.full.scope`, **default false**) selects the scope in `GoogleAuthManager.DRIVE_SCOPE`: off → `drive.file` (production, no CASA); on → full `drive` for the migration build given to existing testers. Don't hand-edit the constant.

**Migration for existing data** (created under the old project/client, invisible to `drive.file`): a one-time **full-`drive`** build server-side-copies (`files.copy`, instant) the legacy folder into a fresh app-owned folder so the copies are created by `923211051414` and become `drive.file`-readable. See `data/drive/DriveMigration.kt`.

**Auto-detect, no picker.** Onboarding's `DriveSignInPage` shows the import section only on the full-`drive` build (`DriveImportSection`, gated by `BuildConfig.DRIVE_FULL_SCOPE`). On sign-in it calls `DriveMigrationViewModel.detect()` → `DriveRepository.findLegacyMigrationSource()` (searches name=`LibreLookAI`, picks the oldest non-trashed folder that holds signature data — a `SYSTEM_JSON_NAMES` file or a `NON_CLOSET_SUBFOLDER_NAMES` subfolder — and is **not** one of our own marked roots) and auto-starts the copy with a progress UI. The Google Picker (`DriveFolderPicker`) now survives only as an on-error manual fallback.

**App-root marker + lost-access recovery.** Every root THIS app creates (fresh-create in `getOrCreateFolder`, or the migration target via `createRootFolderForced`) is stamped with `appProperties[APP_ROOT_PROP="llaiAppRoot"]="1"`. This makes recovery automatic, no UI:
- `getOrCreateFolder` verifies a stored root is still reachable (`folderAccessible` HEAD; 404/403/trashed ⇒ clear the stranded pref and re-resolve — never strand the user on an empty wardrobe). Verified once per repo instance (`rootVerified`).
- With no stored root (reinstall / new device), it re-adopts a **marked** `LibreLookAI` folder. Under `drive.file` an *unmarked* name match is necessarily app-created (the scope can't see foreign files), so it's back-filled with the marker and adopted too — this recovers pre-marker migrated roots. Under full `drive` an unmarked match may be foreign, so it's returned for reads but not claimed (migration decides).
- `findLegacyMigrationSource` skips marked roots, so an already-migrated account never re-migrates on reinstall.
