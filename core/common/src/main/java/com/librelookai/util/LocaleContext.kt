...
import com.librelookai.settings.PreloadedUiPrefs
 ...
 * ```
 *
 * Reads the last-saved language synchronously from [PreloadedUiPrefs.cachedLanguage], so it is
...
fun Context.localized(): Context {
    val locale = AppLanguage.toLocale(PreloadedUiPrefs.cachedLanguage(this))
