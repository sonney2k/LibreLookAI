 ...
 */
val FilterColorKeys: List<String> = listOf(
 ...
 */
fun Iterable<String>.sortedByColorOrder(): List<String> =
...
fun colorSwatchOrNull(name: String): Color? {
    val n = name.lowercase().trim()
    return if (n in FilterColorKeySet) colorSwatchHex(n) else null
}

fun colorSwatchHex(name: String): Color = when (name.lowercase().trim()) {
    // Neutrals
