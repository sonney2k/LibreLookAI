...
2. **Confirmation**: `WardrobeUiState.auditProgress` enters `awaitingConfirmation` and exposes the full list of findings as `items: List<AuditFileEntry>` plus `selectedAuditIds` (defaults to every item). When at least one finding exists, `SettingsScreen` shows the full-screen `RepairPreviewDialog` — a wardrobe-style `LazyVerticalGrid` (96 dp adaptive cells) split into three sections (Orphaned originals, Raw images, Cutouts needing tags). Each tile lazily downloads its preview via `WardrobeViewModel.fetchAuditThumbnail(entry)` (which falls back to `DriveRepository.cachedFile` before hitting the network). Tapping a tile toggles its inclusion via `toggleAuditSelection(driveId)`; section headers have a per-section select-all/deselect-all toggle, and the top selection bar shows total selected/total counts plus global Select all / Deselect all buttons. The bottom action bar shows the total estimated credit cost and a "Process N" button. With zero findings the original `AlertDialog` "all OK / renamed-only" message is still used.
3. **Process** (`continueRepairProcessing(true)`): only items whose Drive ID is in `selectedAuditIds` are processed — orphaned originals get full AI processing (bg removal + tagging + sidecar upload); cutouts missing a sidecar or with empty tags get tagging (from the cutout image) + sidecar upsert. If the user deselects everything, the call is treated as "skip" (cache clear + reload only).
