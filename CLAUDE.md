...
* **Semantic Text Search**: `LocalSemanticSearchHelper` wraps MediaPipe `TextEmbedder` (English Universal Sentence Encoder) + ML Kit Translate for cross-language search. Model lives at `app/src/main/assets/universal_sentence_encoder.tflite` (not vendored — must be downloaded separately); ML Kit's per-language translation packs download on first non-English search. Pipeline: if `sourceLanguage != "en"`, the query and each item's flattened tag document are translated to English via ML Kit, then embedded. The active source language is plumbed from `WardrobeViewModel.setLanguage` (which mirrors `geminiLanguage`) — currently `en` / `de` per `AppLanguage.toBcp47`. Each `DriveImage`'s tags are flattened into a single space-separated document string (label, type, category, uses, colors, seasonality, aesthetic, fit, material, pattern); both the translated string and its embedding are cached in-memory by `driveId:version:srcLang`. Cosine similarity > 0.65 wins, sorted desc. Entry point: wardrobe header's `ImageSearch` icon opens `WardrobeSearchChooserDialog` — text query routes through `WardrobeViewModel.searchByText`, "Search by photo" falls back to the legacy capture flow. Results render in the existing `FindByPhotoResultsSheet`. If the embedder asset is missing, the helper logs and returns an empty list — the photo path keeps working. If translate-model download fails (no network on first use), the helper falls back to embedding the source-language text directly.
