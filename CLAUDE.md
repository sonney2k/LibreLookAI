...
- **Header cluster in fullscreen viewers**: the closet selector + Insights + Settings cluster is reused inside fullscreen `Dialog` viewers (`FullScreenViewer`, `OutfitFullScreenViewer`) and the try-on composer via `ViewerHeaderActions(onBeforeNavigate)` (MainActivity.kt). It reads three CompositionLocals provided at the `MainActivity` top level: `LocalClosetSelector` (`ClosetSelectorContext` — interactive closet dropdown, switches the active closet in place), `LocalOpenInsights`, and `LocalOpenSettings`. Insights/Settings nav runs `onBeforeNavigate` first (dismiss the host Dialog) and both openers also call `tryOnViewModel.close()` since the composer is hosted outside `when(selectedTab)`. Each child self-hides when its local is unprovided (closet dropdown also hides below 2 closets).
