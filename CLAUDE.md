...
- **Optimistic writes**: mutations (move / delete / tag-edit / bg-queue persist) update in-memory state **and** the local cache **synchronously before** the Drive call; Drive syncs after and rolls back on failure. **Cache-folder rule**: key every cache write off the item's *owning* `folderId`, never the transient active-filter `folderId` (null in All-locations, changes on closet switch) — use the file-level helpers (`persistItemToCache` / `addToCacheFile` / `removeFromCacheFile`), don't rebuild a folder cache from the whole `state.images`. Outfits call `refreshOutfitsLocalCache()` after each mutation. Rationale → `plan/CLAUDE_ARCHIVE.md` § Two-phase load / Optimistic writes.
