...
- **402 → typed exception**: `GeminiRepository.throwIf402` parses `{needed, have}` from proxy 402 responses and raises `billing/InsufficientCreditsException`. Every Gemini caller catches it specifically (rethrown above the generic `catch (e: Exception)` swallow) and surfaces an `InsufficientCreditsDialog`.
