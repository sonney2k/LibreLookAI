...
* **Shopping Closet**: Uses a dedicated `_shopping/` folder and is intentionally excluded from standard location lists. Shopping List items participate in selection-mode actions (Create outfit, Try-on me, Move to closet, Delete), and tapping a tile opens the shared `FullScreenViewer` with `writeMode = false` to hide tag/bg/rotate affordances that don't apply to wishlist items. `OutfitComposerScreen` and `TryOnComposerScreen` merge `wardrobe.images + shoppingClosetState.items` so seed IDs from either screen resolve.
