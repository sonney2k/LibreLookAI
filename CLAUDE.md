...
- **First-run onboarding**: a `showOnboarding` flag (seeded from `onboarding/OnboardingState.isComplete`, persisted in its own SharedPrefs file) drives an opaque fullscreen `OnboardingScreen`. A swipeable `HorizontalPager`: value-prop / feature pages → **Connect Google Drive** (`DriveSignInPage`) → **Allow background processing** (`BackgroundPermissionPage`) → **Gemini API key** (`ApiKeyPage`) → style profile → try-on photo → finish. Profile/photo come *after* Drive sign-in because they write to Drive. **Three required steps** (no top-right **Skip**, **Next** soft-blocked until satisfied): Drive sign-in (until `isSignedIn`), background processing (until `PowerManager.isIgnoringBatteryOptimizations`, re-checked on `ON_RESUME`; the global battery `AlertDialog` in `AppContent` stays as a safety net), and the API key (until `startsWith("AIza")` & len ≥ 30, persisted live via `ApiKeyStore`). Every other step is skippable. Re-launchable from **Settings ▸ More ▸ "Take the tour"** via the `LocalStartTour` CompositionLocal (`MainCompositionLocals.kt`) — on replay the Drive/background pages render already-satisfied (✓), and finishing remounts the app with state intact (VMs are activity-scoped, `selectedTab` is `rememberSaveable`). Re-running does **not** clear the persisted flag.
